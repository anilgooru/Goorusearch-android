...
import org.ednovo.shutterbug.DiskLruCache.Editor;
import org.ednovo.shutterbug.DiskLruCache.Snapshot;
...
public class ImageCache {
	public interface ImageCacheListener {
		void onImageFound(ImageCache imageCache, Bitmap bitmap, String key,
				DownloadRequest downloadRequest);

		void onImageNotFound(ImageCache imageCache, String key,
				DownloadRequest downloadRequest);
	}
	...
	public void queryCache(String cacheKey, ImageCacheListener listener,
			DownloadRequest downloadRequest) {
		if (cacheKey == null) {
		...
		if (mDiskCache != null) {
			new BitmapDecoderTask(cacheKey, listener, downloadRequest)
					.execute();
			return;
		}
		listener.onImageNotFound(this, cacheKey, downloadRequest);
	...
	private class BitmapDecoderTask extends AsyncTask<Void, Void, Bitmap> {
		...
		public BitmapDecoderTask(String cacheKey, ImageCacheListener listener,
				DownloadRequest downloadRequest) {
			mCacheKey = cacheKey;
			mListener = listener;
		...
		protected Bitmap doInBackground(Void... params) {
			try {
				...
				if (snapshot != null) {
					try {
						return BitmapFactory.decodeStream(snapshot
								.getInputStream(0));
		...
		protected void onPostExecute(Bitmap result) {
			if (result != null) {
				...
				mListener.onImageFound(ImageCache.this, result, mCacheKey,
						mDownloadRequest);
			} else {
				mListener.onImageNotFound(ImageCache.this, mCacheKey,
						mDownloadRequest);
			}
		}
	...
	private void openDiskCache() {
		...
		try {
			mDiskCache = DiskLruCache.open(directory, versionCode,
					DISK_CACHE_VALUE_COUNT, DISK_CACHE_MAX_SIZE);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
